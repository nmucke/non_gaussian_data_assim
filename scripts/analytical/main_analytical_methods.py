...
(``scripts/main.py``) do **not** see the true posterior -- RMSE / MAE / NRMSE
...
def main(cfg: DictConfig) -> None:
    ...
    da_methods = {
        name: instantiate(
            c, R=R, obs_operator=obs_operator, forward_operator=forward_model
        )
...
TRUTH_ONLY_METRICS = ["rmse", "mae", "nrmse", "crps", "spread", "chi_squared"]
...
def _print_summary(da_methods, metrics) -> None:
    ...
    header = f"{'method':<16}" + "".join(
        f"{(c + ('*' if c in TIME_MEAN_METRICS else '')):>{w}}" for c in cols
    )
    print(header)
    print("-" * len(header))
    for name in da_methods:
    ...
    print(
        "(truth-only metrics: rmse, crps, spread, chi_squared | "
...
def _plot(
    ...
    kf_std = np.sqrt(
        np.clip(np.asarray(jnp.diagonal(kf_cov_arr, axis1=1, axis2=2)), 0.0, None)
    )
    final_ens = {
        n: np.asarray(ens_arr[n][:, -1]).reshape(n_ens, -1) for n in da_methods
    }
    ...
    fig, axes = plt.subplots(
        n_plot_dims, 1, figsize=(11, 3.0 * n_plot_dims), squeeze=False
    )
    for d in range(n_plot_dims):
        ...
        ax.plot(
            steps, kf_mean[:, d], color="tab:red", lw=2, label="Kalman (analytical)"
        )
        ax.fill_between(
            steps,
    ...
    fig.suptitle(
        f"State trajectories: truth, analytical Kalman filter, ensemble methods  "
    ...
    fig, axes = plt.subplots(
        nm,
        n_plot_dims,
        figsize=(6.0 * n_plot_dims, 2.6 * nm),
        squeeze=False,
        sharex=True,
    )
    ...
    fig.suptitle(
        f"Per-method trajectory with ±2σ uncertainty bands  "
    ...
    for ax in axes.flat:
        ax.axis("off")
    kf_spread = np.sqrt(
        np.mean(np.asarray(jnp.diagonal(kf_cov_arr, axis1=1, axis2=2)), axis=1)
    )
    for ax, key in zip(axes.flat, panels):
        ...
        if key == "spread":
            ax.plot(
                steps, kf_spread, color="tab:red", lw=2, label="Kalman (analytical)"
            )
            ax.legend(fontsize=8)
        if key in POSTERIOR_METRICS:
            ax.set_yscale("symlog", linthresh=1e-3)
        ax.set_title(
            METRIC_TITLES.get(key, key)
            + ("" if truth_only else "  [needs true posterior]"),
            fontsize=9,
            color=("black" if truth_only else "tab:red"),
        )
        ax.set_xlabel("assimilation step")
        ax.grid(alpha=0.3)
    axes.flat[0].legend(fontsize=8)
    fig.suptitle(
        "Diagnostics per assimilation step — truth-only (black titles) vs "
    ...
    for ax, key in zip(axes.flat, TRUTH_ONLY_METRICS):
        ...
        ax.set_title(
            f"{METRIC_TITLES.get(key, key)}  vs  true-posterior KL", fontsize=9
        )
        ax.grid(alpha=0.3)
    axes[0, 0].legend(fontsize=8)
    fig.suptitle(
        "Is the cheap (truth-only) metric a good surrogate for the "
    ...
    fig, axes = plt.subplots(
        1,
        len(da_methods),
        figsize=(4.6 * len(da_methods), 3.8),
        squeeze=False,
        sharex=True,
        sharey=True,
    )
    ...
    axes[0, 0].legend(fontsize=8)
    fig.suptitle(
        "Innovation whitening check — z should be ~N(0,1) if forecast spread + R are consistent"
    )
    fig.tight_layout()

    # ===== Figure 6: final-step posterior marginals (first 2 dims) ======== #
    fig, axes = plt.subplots(
        1, n_plot_dims, figsize=(5.0 * n_plot_dims, 4.0), squeeze=False
    )
    for d in range(n_plot_dims):
        ax = axes[0, d]
        ...
        ax.plot(
            gx,
            np.exp(-0.5 * ((gx - mu) / sigma) ** 2) / (sigma * np.sqrt(2 * np.pi)),
            color="tab:red",
            lw=2.5,
            label="analytical Kalman",
        )
        for name in da_methods:
            ax.hist(
                final_ens[name][:, d],
    ...
    fig.suptitle(
        f"Final-step posterior marginals vs analytical Kalman  "
        f"(showing first {n_plot_dims} of {state_dim} state dims)"
    )
    fig.tight_layout()
    ...
    if n_plot_dims >= 2:
        ...
        nm = len(da_methods)
        fig, axes = plt.subplots(
            1, nm, figsize=(5.0 * nm, 4.8), squeeze=False, sharex=True, sharey=True
        )
        for ax, name in zip(axes[0], da_methods):
            ax.scatter(
                ana_f[:, 0],
            ...
            for n_std, ls in ((1.0, "-"), (2.0, "--")):
                cov_ellipse(
                    ax, m_kf_2d, P_kf_2d, n_std, edgecolor="tab:red", lw=2, ls=ls
                )
                cov_ellipse(
                    ax,
                    mu_p_2d,
                    sig_p_2d,
                    n_std,
                    edgecolor=_color(name, da_methods),
                    lw=2,
                    ls=ls,
                )
            ax.set_title(name)
        ...
        axes[0, 0].legend(fontsize=8)
        fig.suptitle(
            f"Final-step joint posterior in state[0]-state[1]: ensemble vs "
    ...
    fig.suptitle(
        "Observation-space diagnostics: predicted observations vs noisy observations"
    )
    fig.tight_layout()
    ...
    for ax, (obs_key, state_key) in zip(axes.flat, pairings):
        ...
        for name in da_methods:
            ...
            if y.shape[0] == x.shape[0] + 1:
                y = y[1:]
            ax.scatter(
                x, y, s=25, color=_color(name, da_methods), label=name, alpha=0.8
            )
        ax.set_xlabel(METRIC_TITLES.get(obs_key, obs_key) + "\n(obs-only)", fontsize=8)
        ax.set_ylabel(
            METRIC_TITLES.get(state_key, state_key) + "\n(full-state, true posterior)",
            fontsize=8,
        )
    ...
    axes.flat[0].legend(fontsize=8)
    fig.suptitle(
        "Reliability of obs-only metrics: each obs-only diagnostic plotted "
    ...
    for ax, key in zip(axes.flat, obs_metrics):
        ...
        for name in da_methods:
            ...
            ax.scatter(
                x, y[1:], s=25, color=_color(name, da_methods), label=name, alpha=0.8
            )
        ax.set_yscale("symlog", linthresh=1e-3)
        ax.set_xlabel(METRIC_TITLES.get(key, key) + "\n(obs-only)", fontsize=8)
    ...
    axes.flat[0].legend(fontsize=8)
    fig.suptitle(
        "Obs-only metrics vs the gold-standard true-posterior KL  "
