...
def _print_summary(sizes, metrics, method_name) -> None:
    ...
    w = 16
    header = f"{'N':<6}" + "".join(
        f"{(c + ('*' if c == 'chi_squared' else '')):>{w}}" for c in cols
    )
    print(header)
    print("-" * len(header))
    for N in sizes:
...
def _size_colors(sizes):
    norm = (
        mcolors.LogNorm(vmin=min(sizes), vmax=max(sizes))
...
def _plot(
    ...
    fig.suptitle(
        f"Per-ensemble-size trajectory with ±2σ uncertainty bands "
    ...
    fig.suptitle(
        f"Metric value vs ensemble size  ({method_name})  -- "
    ...
    for ax, key in zip(axes.flat, OBS_METRICS):
        ...
        for N in sizes:
            ax.plot(
                obs_steps, metrics[N][key], "-o", ms=3, color=colors[N], label=f"N={N}"
            )
    ...
    for ax, (obs_key, state_key) in zip(axes.flat, pairings):
        ...
        ax.set_ylabel(
            METRIC_TITLES.get(state_key, state_key) + " (full-state)", fontsize=8
        )
        ax.set_title(f"{obs_key}  vs  {state_key}", fontsize=9)
        ax.grid(alpha=0.3)
    axes.flat[0].legend(fontsize=7)
    fig.suptitle(
        f"Reliability of obs-only metrics across ensemble sizes  ({method_name})\n"
    ...
    axes.flat[0].legend(fontsize=7)
    fig.suptitle(
        f"Obs-only metrics vs the gold-standard true-posterior KL  "
    ...
    fig.suptitle(
        f"Final-step posterior marginals across ensemble sizes  ({method_name})"
    )
    fig.tight_layout()
    ...
    if n_plot_dims >= 2:
        ...
        ncol = len(sizes)
        fig, axes = plt.subplots(
            1, ncol, figsize=(4.5 * ncol, 4.5), squeeze=False, sharex=True, sharey=True
        )
        for ax, N in zip(axes[0], sizes):
            ...
            for n_std, ls in ((1.0, "-"), (2.0, "--")):
                ...
                cov_ellipse(
                    ax, mu_p_2d, sig_p_2d, n_std, edgecolor=colors[N], lw=2, ls=ls
                )
            ax.set_title(f"N = {N}")
        ...
        fig.suptitle(
            f"Final-step joint posterior state[0]-state[1] vs analytical "
