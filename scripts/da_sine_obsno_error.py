...
def main() -> None:
    ...
    rng_key, key = jax.random.split(rng_key)
    prior_ensemble = (
        prior_ensemble.at[:, 0, 0].set(jax.random.normal(key, (ENSEMBLE_SIZE,))) + 1.0
    )
    rng_key, key = jax.random.split(rng_key)
    ...
    kde = gaussian_kde(posterior_ensemble, bw_method="silverman")
